...
public class ReviewDAO {
     ...
     * 
     * @param roomIdx 해당 숙소의 ID
     * @return 조회수 기준으로 정렬된 리뷰 리스트(List<ReviewDTO>)
     */
    public List<ReviewDTO> getReviewsByRoomIdxSortedByViewCount(int roomIdx) {
        List<ReviewDTO> reviews = new ArrayList<>();
        String sql = "SELECT * FROM reviews WHERE room_idx = ? ORDER BY view_count DESC, created_at DESC";
        ...
        try (Connection conn = HomesDB.getConn();
            ...
            while (rs.next()) {
                ...
                review.setViewCount(rs.getInt("view_count"));
